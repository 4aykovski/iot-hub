...
func (d *Device) GetDevice(ctx context.Context, id string) (model.Device, error) {
	...
	stmt, args, err := squirrel.Select("*").From("devices").Where("id = ?", id).ToSql()
	if err != nil {
		return model.Device{}, fmt.Errorf("device.GetDevice: %w", err)
	}

	var device model.Device
	err = conn.QueryRow(ctx, stmt, args...).Scan(&device.ID, &device.Name, &device.Limit)
	if err != nil {
...
func (d *Device) GetDevices(ctx context.Context) ([]model.Device, error) {
	...
	stmt, args, err := squirrel.Select("*").From("devices").ToSql()
	if err != nil {
	...
	for rows.Next() {
		var device model.Device
		err = rows.Scan(&device.ID, &device.Name, &device.Limit)
		if err != nil {
			return []model.Device{}, fmt.Errorf("device.GetDevices: %w", err)
		}
